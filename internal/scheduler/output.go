...
func renderHumanReport(source Source, contract Contract, contractDigest, fixtureDigest, casesDigest string, cases []CaseResult, baseline, candidate RunResult, comparison PairComparison, replay ReplayReceipt, outputs []OutputDigest) string {
	...
	fmt.Fprintf(&builder, "Critical exactness: status=%t, digest=%t, UNKNOWN six fields=%t, REFUTED counterexamples=%t, final semantic root=%t, actual wall reduction=%t.\n\n", comparison.PerLockStatusExact, comparison.DigestExact, comparison.UnknownFieldsExact, comparison.CounterexamplesExact, comparison.FinalSemanticRootExact, comparison.WallReductionActual)
