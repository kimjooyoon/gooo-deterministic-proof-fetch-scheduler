...
test -f "${source_file}"
test "$(grep -c '^lock ' "${source_file}")" = 53
test "$(grep -c '^case ' "${source_file}")" = 9
test "$(grep -c '^canonical ' "${source_file}")" = 1
test "$(grep -c '^concurrency ' "${source_file}")" = 1
test "$(grep -c '^retry ' "${source_file}")" = 1
...
! grep -RInE 'github\.com|api\.github|curl |wget |go test|go build|go vet|gofmt' "internal" "examples" "fixtures" >/dev/null
